...
func TestInvitationBuilderPersistsClaimsAndReusesExactBearer(t *testing.T) {
	...
	attempt := store.DeliveryAttempt{PollID: "poll", ParticipantID: "person", RecipientEmails: []string{"reader@example.test", "alias@example.test"}, MessageKind: "invitation", Question: "Question"}
	...
	if len(first.To) != 2 || first.To[0] != "reader@example.test" || first.To[1] != "alias@example.test" {
		t.Fatalf("recipients=%v", first.To)
	}
	var count int
